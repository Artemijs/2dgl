...
void Utility::DrawSpriteManually(const Matrix4x4& matrix, const BaseMaterial* bm) {
...
void Utility::DrawSpriteManually(const Vec3& pos, const Vec3& scale, const BaseMaterial* bm) {
	Matrix4x4 m = (Matrix4x4::TranslationMatrix(pos) * Matrix4x4::ScaleMatrix(scale));
	bm->Bind(&m);
	Renderer::instance()->GetVAO()->Bind();
	glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
	bm->Unbind();
