...
Text::Text(Vec3 position, float angle, int fontSize, std::string txt, int maxCharPerLine):
	Graphic("Fonts/arial.png", SpriteData()), _text(txt), _maxCharsPerLine(maxCharPerLine), _fontSize(fontSize) {
	if (txt.length() == 0) {
		_text = " ";
	}
...
void Text::Init() {
	std::ifstream input("Fonts/arial.fnt");
	int count = -1;
	float texW = 512;
	float texH = 512;
...
void Text::SetFontSize(int size) {
	...
	float scaler = 20;
	_fontSize = size;
	_rect._size = Vec2(baseSize + size * scaler, baseSize + size * scaler);
}
void Text::SetSize(Vec2 size) {
	_rect._size = size;
}
void Text::SetPosition(Vec3 pos) {
	_rect._position = pos;
}
void Text::SetAngle(float angle) {
	_rect._angle = angle;
}
SpriteData Text::GetRect() {
	return _rect;
}
