...
UIAnimatedSprite::~UIAnimatedSprite() {}
UIAnimatedSprite::UIAnimatedSprite(const char* path, Vec3 pos, Vec3 size, float ang) :
	UIsprite(path, pos, size, ang),
	_texScale(Vec2()),
	_looping(false),
	_currentFrame(0),
	_startEndFrames(Vec2Int(0, 1)),
...
void UIAnimatedSprite::SetAnimationData(int startFrame, int length, int maxXframes, int maxYframes, float timePerFrame, bool looping) {
...
void UIAnimatedSprite::BeforeDraw() {
	UIsprite::BeforeDraw();
