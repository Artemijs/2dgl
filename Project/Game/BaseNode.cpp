...
BaseNode::BaseNode(const Vec3 pos, const Vec3 size, const float ang){
...
BaseNode::BaseNode(const Vec3 pos, const Vec3 size, const Vec3 ang):
	_components(new std::vector<std::pair<const unsigned int, BaseComponent*>*>()),
	_transform({pos, size, ang}),
	_parent(NULL),
	_children(new std::vector<BaseNode*>()),
	_visible(true),
	_model(Matrix4x4(1.0f))
